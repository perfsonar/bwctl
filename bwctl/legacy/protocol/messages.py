...
#class ProtocolBuffer(bytearray):
#    def __init__(self, message=""):
#        self[:] = message
#
#    def get_ntohl(self, location):
#        return struct.unpack("!L", self[location:location + 3])[0]
#
#    def get_ntohs(self, location):
#        return struct.unpack("!H", self[location:location + 1])[0]
#
#    def set_ntohl(self, location, value):
#        self[location:location + 3] = pack("! L", value)
#
#    def set_ntohs(self, location, value):
#        self[location:location + 1] = pack("! H", value)
...
class ServerGreeting:
    def __init__(self, modes=[], protocol_version=5, challenge=''):
        self.modes = modes
...
class ClientGreeting:
    def __init__(self, protocol_version=5, mode=1, username='', token='', client_iv=''):
        self.protocol_version = protocol_version
        self.mode = mode
    ...
    @staticmethod
    def parse(data):
        (mode, username, token, client_iv) = unpack("! L 16s 32s 16s", data)
...
class StopSession:
    ...
    def unparse(self, include_msg_hdr=True):
        if include_msg_hdr:
            return pack("! B B 6x L 2Q", MessageTypes.StopSession, self.result_status, self.result_length, 0, 0)
        else:
            return pack("! B 6x L 2Q", self.result_status, self.result_length, 0, 0)
